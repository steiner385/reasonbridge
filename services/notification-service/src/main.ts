...
async function bootstrap() {
  const app = await NestFactory.create<NestFastifyApplication>(AppModule, new FastifyAdapter(), {
    ...
    logger: new PinoLoggerService({ name: 'notification-service' }),
  });
  ...
  const config = new DocumentBuilder()
    .setTitle('ReasonBridge Notification Service')
    .setDescription('Real-time notifications, email, and push notification endpoints')
    .setVersion('1.0.0')
    .addBearerAuth()
    .build();
