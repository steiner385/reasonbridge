...
export class QueueModule implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    // Defensive check - queueService may not be injected during module initialization
    if (!this.queueService) {
      console.warn('QueueService not available during module init, skipping queue initialization');
      return;
    }
