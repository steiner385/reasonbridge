...
async function main() {
  ...
  const topic1 = existingTopic1 || await prisma.discussionTopic.create({
    data: {
      title: 'Should renewable energy be prioritized over fossil fuels?',
      description: 'Discuss the transition to renewable energy sources and their impact on the environment and economy.',
  ...
  const topic2 = existingTopic2 || await prisma.discussionTopic.create({
    data: {
      title: 'Universal Basic Income: Viable or Unsustainable?',
      description: 'Explore the feasibility and potential impact of implementing universal basic income policies.',
  ...
  const topic3 = existingTopic3 || await prisma.discussionTopic.create({
    data: {
      title: 'Remote Work: The Future of Employment?',
      description: 'Debate the long-term effects of remote work on productivity, work-life balance, and urban development.',
      creatorId: testUser1.id,
      crossCuttingThemes: [],
    },
  });
  ...
  const response1 = existingResponse1 || await prisma.response.create({
    data: {
      topicId: topic1.id,
      authorId: testUser2.id,
      content: 'Renewable energy is crucial for reducing carbon emissions and combating climate change. The initial investment costs are offset by long-term savings and environmental benefits.',
      citedSources: [],
    },
  });
  ...
  const response2 = existingResponse2 || await prisma.response.create({
    data: {
      topicId: topic1.id,
      authorId: testUser1.id,
      parentId: response1.id,
      content: 'While I agree on the environmental benefits, we need to consider the economic transition costs and job displacement in fossil fuel industries. A gradual transition with retraining programs would be more sustainable.',
      citedSources: [],
    },
  });
