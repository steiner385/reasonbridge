...
describe.skipIf(!process.env['DATABASE_URL'])('Query Performance - Index Verification', () => {
  let pool: Pool;
  ...
  describe('Discussion Topic Queries', () => {
    it('should use index for topic lookup by slug', async () => {
      const result = await explainAnalyze(pool, 'SELECT * FROM discussion_topics WHERE slug = $1', [
        'perf-test-topic',
      ]);
