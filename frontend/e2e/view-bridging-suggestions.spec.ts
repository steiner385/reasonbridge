...
test.describe('View Bridging Suggestions', () => {
  ...
  test('should display bridging language text in suggestions', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      if (cardCount > 0) {
        ...
        const hasText = await bridgingText.count() > 0;
  ...
  test('should display common ground explanation in suggestion cards', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      if (cardCount > 0) {
        ...
        const commonGroundText = firstCard.locator('[data-testid="common-ground-text"]');
  ...
  test('should highlight medium confidence suggestions (60-79%) in blue', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      const mediumConfidenceBadges = page.locator('[data-testid="confidence-medium"]');
  ...
  test('should display analysis reasoning at top of suggestions panel', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      if (hasSection) {
        const analysisReasoning = suggestionsSection.locator('[data-testid="analysis-reasoning"]');
  ...
  test('should display empty state when no bridging suggestions available', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      if (hasSection) {
        ...
        const emptyMessage = suggestionsSection
  ...
  test('should display AI attribution footer', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      if (hasSection) {
        ...
        const attribution = suggestionsSection.locator('[data-testid="ai-attribution"]');
  ...
  test('should update bridging suggestions in real-time via WebSocket', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      const suggestionsSection = page.locator('[data-testid="bridging-suggestions"]');
      const initialContent = await suggestionsSection.textContent().catch(() => '');
  ...
  test('should display maximum suggestion count limit if configured', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      const countIndicator = page.locator('[data-testid="suggestion-count-indicator"]');
  ...
  test('should distinguish suggestion cards from agreement zones and divergence', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      const agreementZones = page.locator('[data-testid="agreement-zone"]');
      const divergencePoints = page.locator('[data-testid="divergence-points"]');
      const suggestions = page.locator('[data-testid="bridging-suggestions"]');
  ...
  test('should provide visual hierarchy for suggestion importance', async ({ page }) => {
    ...
    if (linkCount > 0) {
      ...
      const highConfidenceCards = page.locator('[data-testid="bridging-suggestion-card"]').filter({
