...
export interface CommonGroundSummaryPanelProps {
  ...
  /**
   * Optional callback when agreement zone is clicked to highlight related responses
   */
...
const CommonGroundSummaryPanel = ({
  ...
  return (
    <div className={`space-y-6 ${className}`} data-testid="common-ground-summary">
        ...
        <div className="grid grid-cols-3 gap-4">
          {analysis.agreementZones.length > 0 && (
            <div className="bg-green-50 rounded-lg border border-green-200 p-4 text-center">
              ...
              <div className="text-sm text-green-700">Agreement Zones</div>
            </div>
          )}
          {analysis.misunderstandings.length > 0 && (
            <div className="bg-purple-50 rounded-lg border border-purple-200 p-4 text-center">
              ...
              <div className="text-sm text-purple-700">Misunderstandings</div>
            </div>
          )}
          {analysis.disagreements.length > 0 && (
            <div className="bg-blue-50 rounded-lg border border-blue-200 p-4 text-center">
              <div className="text-2xl font-bold text-blue-800">
                {analysis.disagreements.length}
              </div>
              <div className="text-sm text-blue-700">Disagreements</div>
            </div>
          )}
        </div>
      )}

      {/* Agreement Zones (detailed view) */}
      {showFullAnalysis && analysis.agreementZones.length > 0 && (
        <div
        ...
        >
          ...
          <div className="space-y-3">
            {analysis.agreementZones.map((zone) => {
              ...
              const hasClickHandler = Boolean(onAgreementZoneClick);
              return (
                <div
                  key={zone.id}
                  ...
                  role={hasClickHandler ? 'button' : 'article'}
                ...
                >
                  ...
                  {hasClickHandler &&
                    zone.relatedResponseIds &&
                    zone.relatedResponseIds.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-300">
                        <p className="text-xs text-gray-600">
                          Click to view {zone.relatedResponseIds.length} related response
                          {zone.relatedResponseIds.length !== 1 ? 's' : ''}
                        </p>
                      </div>
                    )}

                  {onViewAgreementZone && !hasClickHandler && (
                    <button
                      type="button"
      ...
      {/* Misunderstandings (detailed view) */}
      {showFullAnalysis && analysis.misunderstandings.length > 0 && (
        <div
          className="bg-white rounded-lg shadow-sm border border-gray-200 p-6"
      ...
      {/* Genuine Disagreements (detailed view) */}
      {showFullAnalysis && analysis.disagreements.length > 0 && (
        <div
          className="bg-white rounded-lg shadow-sm border border-gray-200 p-6"
