/**
 * Copyright 2025 Tony Stein
 * SPDX-License-Identifier: Apache-2.0
 */

import React from 'react';
import type { Topic } from '../../types/topic';

/**
 * TopicListItem props
 */
export interface TopicListItemProps {
  /** Topic data to display */
  topic: Topic;
  /** Whether this topic is currently selected/active */
  isActive?: boolean;
  /** Click handler for topic selection */
  onClick?: (topicId: string) => void;
  /** Whether to show an unread badge */
  hasUnread?: boolean;
  /** CSS class name */
  className?: string;
}

/**
 * Compact topic list item for sidebar navigation
 * Optimized for space efficiency in left panel
 */
export function TopicListItem({
  topic,
  isActive = false,
  onClick,
  hasUnread = false,
  className = '',
}: TopicListItemProps) {
  const getStatusColor = (status: Topic['status']) => {
    switch (status) {
      case 'ACTIVE':
        return 'text-green-600';
      case 'SEEDING':
        return 'text-yellow-600';
      case 'ARCHIVED':
        return 'text-gray-400';
      default:
        return 'text-gray-400';
    }
  };

  const handleClick = () => {
    if (onClick) {
      onClick(topic.id);
    }
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      handleClick();
    }
  };

  return (
    <div
      className={`
        topic-list-item
        relative px-3 py-2.5 cursor-pointer rounded-md transition-colors
        ${isActive ? 'bg-primary-100 text-primary-900' : 'hover:bg-gray-100 text-gray-900'}
        ${className}
      `}
      data-testid="topic-list-item"
      data-topic-id={topic.id}
      onClick={handleClick}
      onKeyDown={handleKeyDown}
      role="button"
      tabIndex={0}
      aria-label={`Select topic: ${topic.title}`}
      aria-current={isActive ? 'page' : undefined}
    >
      {/* Unread indicator */}
      {hasUnread && (
        <div
          className="absolute left-1 top-1/2 -translate-y-1/2 w-2 h-2 bg-primary-600 rounded-full"
          aria-label="Unread messages"
        />
      )}

      {/* Topic title */}
      <div className="flex items-start justify-between gap-2 mb-1">
        <h3
          className={`text-sm font-medium line-clamp-2 flex-1 ${hasUnread ? 'pl-3' : ''}`}
          title={topic.title}
        >
          {topic.title}
        </h3>

        {/* Status indicator dot */}
        <div
          className={`flex-shrink-0 w-1.5 h-1.5 rounded-full mt-1.5 ${getStatusColor(topic.status)}`}
          title={`Status: ${topic.status}`}
          aria-label={`Status: ${topic.status}`}
        />
      </div>

      {/* Metadata row */}
      <div className="flex items-center gap-3 text-xs text-gray-600">
        {/* Participant count */}
        <div className="flex items-center gap-1" title={`${topic.participantCount} participants`}>
          <svg
            className="w-3.5 h-3.5"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
            />
          </svg>
          <span>{topic.participantCount}</span>
        </div>

        {/* Response count */}
        <div className="flex items-center gap-1" title={`${topic.responseCount} responses`}>
          <svg
            className="w-3.5 h-3.5"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"
            />
          </svg>
          <span>{topic.responseCount}</span>
        </div>

        {/* Diversity score (if available) */}
        {topic.currentDiversityScore !== null && (
          <div
            className="flex items-center gap-1"
            title={`Diversity score: ${topic.currentDiversityScore.toFixed(1)}`}
          >
            <svg
              className="w-3.5 h-3.5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"
              />
            </svg>
            <span>{topic.currentDiversityScore.toFixed(1)}</span>
          </div>
        )}
      </div>
    </div>
  );
}
