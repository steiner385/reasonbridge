...
Build #39+ will automatically:
1. Wait for postgres to accept connections (retry logic with 5 attempts)
2. Apply Prisma migrations to E2E database (port 5434)
3. Seed test data before running Playwright tests
4. Execute all 301 E2E tests with proper configuration

Expected results: 245 passing, 56 skipped (consistent with local testing)

**Note:** Connection retry logic resolves race condition between docker healthcheck
passing and external port being ready for connections
